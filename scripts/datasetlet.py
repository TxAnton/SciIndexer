...
logger = open("data/datasetlet_log.txt","a+")
...
for klass in tqdm(classes):
    ...
    if pr: print("=== Processing class:", klass,file=logger)
    found = list()

    if pr: print("=Searching in APS dataset...",file=logger)
    for i in df_aps.iloc:
        i:Series
        if i.Afflict.lower() == klass:
            # found.add(i["Full plant name"])
            ...
            found.append(i)
    if not found:
        if pr: print(f"-Not found in APS dataset: {klass}",file=logger)
        continue

    fn = found[0]["Full plant name"] # found full name
    if pr: print(f"+Found in APS dataset: {fn}",file=logger)
    # in_aps.append(klass)
    ...
    dataset_dict[klass]["Diseases"] = dict()    
    
    # dataset_dict["Diseases"] = dataset_dict.get("Diseases", dict())
    for f in tqdm(found, leave=False):
        
        dis_name = f["Disease"]

        dataset_dict[klass]["Diseases"][dis_name] = dict()
        # dataset_dict["Diseases"][dis_name]["Pathogens"] = [iss.split("(")[0].replace("Genus","").lower() for iss in f["Pathogen"].split("\n")]
        dataset_dict[klass]["Diseases"][dis_name]["Pathogens"] = []
        pathogens  = [iss.strip().lower().split("(")[0].replace("genus","") for iss in f["Pathogen"].replace(",","\n").split("\n")]
        dataset_dict[klass]["Diseases"][dis_name]["Afflict"] = f["Afflict"]
        ...
        for pathogen in tqdm(set(pathogens), leave=False):
            if pr: print(f"- Searching eppo for pathogen: {pathogen}",file=logger)
            # lru_cache: check if already cached
            
            if skip_pathos:
                res = []
            else:
                _hits = _search_local_eppo_names.cache_info().hits
                
                res = search_local_eppo_name(pathogen) # (name, eppo)
                
                if _search_local_eppo_names.cache_info().hits > _hits:
                    skip_pathos = False
            
            if not res:
                if skip_pathos:
                    if pr: print(f"  - Skipping eppo search for pathogen: {pathogen}",file=logger)
                else:
                    if pr: print(f"  - Not found in eppo: {pathogen}",file=logger)
                pass
            else:
                founds.append((pathogen, res))
                # propper_fullname, eppo = res
                if pr: print(f"  + Found in eppo: {propper_fullname} -> {eppo}",file=logger)

        founds
        for fs in founds:
            pathogen, (propper_fullname, eppo) = fs
            paths = dataset_dict[klass]["Diseases"][dis_name]["Pathogens"] 
            dataset_dict[klass]["Diseases"][dis_name]["Pathogens"].append({
                "Queried": pathogen,
...
#%%
with open ("data/test_datasetlet_6.json", 'w+') as fd:
    json.dump(dataset_dict,fd,indent=4)
#%%

#%%

# paths = [p for p in pathogens for pathogens in dis_d.values for dis_d in data.values]
paths = [p for dis_d in data.values() for p in dis_d.get("Pathogens",[])]
